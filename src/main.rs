...
use gc::bin::{
    convert_bristol_to_compressed_binary, verify_bristol_circuit, verify_compressed_binary_circuit,
};
...
enum Commands {
    ...
    /// Verify Bristol format circuit file and show statistics
    VerifyBristol {
        /// Path to the Bristol circuit file
        #[arg(help = "Bristol circuit file to verify")]
        file: PathBuf,
    },
    /// Convert Bristol format to compressed binary format using zstd
    ConvertToCompressedBinary {
        /// Path to the Bristol circuit file
        #[arg(help = "Bristol circuit file to process")]
        file: PathBuf,
        /// Output file for compressed binary format (default: <input>.zst)
        #[arg(
            short = 'o',
            long = "output",
            help = "Output file for compressed binary format"
        )]
        output: Option<PathBuf>,
    },
    /// Verify compressed binary format circuit file and show statistics
    VerifyCompressedBinary {
        /// Path to the compressed binary circuit file
        #[arg(help = "Compressed binary circuit file to verify")]
        file: PathBuf,
    },
}
...
fn main() -> Result<()> {
    ...
    match args.command {
        ...
        Commands::SingleUseAnalysis {
            ...
            // Print results
            println!(
                "Single-use wires from AND gates: {}",
                single_use_analysis.single_use_and_gates
            );
            println!(
                "Single-use wires from XOR gates: {}",
                single_use_analysis.single_use_xor_gates
            );
            println!(
                "Total single-use wires: {}",
                single_use_analysis.total_single_use_wires
            );
        }
        Commands::MemorySimulation {
            ...
            println!(
                "Memory simulation results saved to: {}",
                output_path.display()
            );
        }
        Commands::Garble {
        ...
        Commands::VerifyBristol { file } => {
            // Verify Bristol circuit format
            println!("Verifying Bristol circuit format...");
            let stats = verify_bristol_circuit(&file)?;

            // Print verification statistics
            stats.print_summary();
            println!("Verification completed: {}", file.display());
        }
        Commands::ConvertToCompressedBinary { file, output } => {
            // Determine output file
            let output_path = output.unwrap_or_else(|| {
                let mut path = file.clone();
            ...
            // Convert Bristol to compressed binary format
            println!("Converting Bristol circuit to compressed binary format...");
            let stats = convert_bristol_to_compressed_binary(&file, &output_path)?;

            // Print conversion statistics
            stats.print_summary();
            println!("Conversion completed: {}", output_path.display());
        }
        Commands::VerifyCompressedBinary { file } => {
            // Verify compressed binary circuit format
            println!("Verifying compressed binary circuit format...");
            let stats = verify_compressed_binary_circuit(&file)?;
