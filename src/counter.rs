...
pub fn count_gate_types(stream: &mut BufferedLineStream) -> Result<HashMap<String, usize>> {
    ...
    let _num_gates: u32 = header_tokens[0].parse()
    ...
    while let Some(line_result) = stream.next_line() {
        ...
        let gate_type = line.split_whitespace().last()
            .ok_or_else(|| anyhow::anyhow!("Empty line or no gate type found at line {}: '{}'", line_number, line))?;
