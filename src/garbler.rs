...
pub fn garble_circuit(
    ...
    let _num_wires: u32 = header_tokens[1].parse()
    ...
    let mut active_wire_labels: std::collections::HashMap<u32, WireLabel> = std::collections::HashMap::new();
    ...
    while let Some(line_result) = stream.next_line() {
        ...
        let input_wires: Result<Vec<u32>> = (0..num_inputs).map(|i| {
        ...
        let output_wires: Result<Vec<u32>> = (0..num_outputs).map(|i| {
            let wire_id: u32 = tokens.next()
        ...
        match gate_type {
            "XOR" => {
                ...
                if input_wires.len() != 2 || output_wires.len() != 1 {
                    bail!("XOR gate must have 2 inputs and 1 output at line {}", line_number);
                }
                
                let input1_label_0 = active_wire_labels.get(&input_wires[0])
                    .ok_or_else(|| anyhow::anyhow!("Input wire {} not found at line {}", input_wires[0], line_number))?;
                let input2_label_0 = active_wire_labels.get(&input_wires[1])
                    .ok_or_else(|| anyhow::anyhow!("Input wire {} not found at line {}", input_wires[1], line_number))?;
            ...
            "AND" => {
                ...
                if input_wires.len() != 2 || output_wires.len() != 1 {
                    bail!("AND gate must have 2 inputs and 1 output at line {}", line_number);
                }
...
mod tests {
    ...
    fn test_garble_and_gate() {
        let input1_labels = [
            WireLabel::new([0x10; 16]),
            WireLabel::new([0x11; 16]),
        ];
        let input2_labels = [
            WireLabel::new([0x20; 16]),
            WireLabel::new([0x21; 16]),
        ];
        let output_labels = [
            WireLabel::new([0x30; 16]),
            WireLabel::new([0x31; 16]),
        ];
    ...
    fn test_wire_labels_json_serialization() -> Result<()> {
        ...
        assert_eq!(loaded_labels.input_labels.len(), wire_labels.input_labels.len());
        assert_eq!(loaded_labels.output_labels.len(), wire_labels.output_labels.len());
    ...
    fn test_deterministic_garbling() -> Result<()> {
        ...
        for (table1, table2) in result1.garbled_tables.iter().zip(result2.garbled_tables.iter()) {
