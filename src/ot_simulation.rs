...
pub fn simulate_ot(wire_labels: &WireLabels, seed_data: &[u8; 32]) -> Result<OTResult> {
    ...
    pb.set_style(
        ProgressStyle::default_bar()
            .template("{bar:40.green/blue} {pos}/{len} inputs [{elapsed_precise}<{eta_precise}] {msg}")
            .unwrap()
            .progress_chars("#>-")
    );
...
mod tests {
    ...
    fn test_ot_simulation_label_consistency() -> anyhow::Result<()> {
        ...
        let mut input_labels = HashMap::new();
        let label_0 = WireLabel::new([0x10, 0x20, 0x30, 0x40, 0x50, 0x60, 0x70, 0x80, 
                                     0x90, 0xA0, 0xB0, 0xC0, 0xD0, 0xE0, 0xF0, 0x00]);
        let delta = WireLabel::new([0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08,
